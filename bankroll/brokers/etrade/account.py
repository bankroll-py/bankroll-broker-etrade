...
class Settings(configuration.Settings):
    ...
    def help(self) -> str:
        if self == self.POSITIONS:
            return "A local path to an exported CSV of Etrade positions."
        elif self == self.TRANSACTIONS:
            return "A local path to an exported CSV of Etrade transactions."
        else:
            return ""
...
class _EtradePosition(NamedTuple):
    symbol: str
    description: str
    quantity: str
    price: str
...
_InstrumentFactory = Callable[[_EtradePosition], Instrument]


def _parseEtradePosition(
    p: _EtradePosition, instrumentFactory: _InstrumentFactory
) -> Position:
...
def _parseOptionsPosition(description: str) -> Option:
    ...
    if not match:
        raise ValueError(f"Could not parse ETrade options description: {description}")
    ...
    month = _EtradeMonth[match["month"]]
...
def _parsePositions(path: Path, lenient: bool = False) -> List[Position]:
    with open(path, newline="") as csvfile:
        ...
        for sec in sections:
            for r in sec.rows:
                pos = _parseEtradePosition(
                    _EtradePosition._make(r), instrumentBySection[sec.criterion]
                )
...
def _parseCash(p: _EtradePosition) -> Cash:
    # Etrade's CSV seems to be formatted incorrectly, with cash price
    ...
    if Decimal(p.quantity) != Decimal(1) or Decimal(p.price) == Decimal(1):
        raise ValueError(f"Etrade cash position format has changed to: {p}")
...
def _parseBalance(path: Path, lenient: bool = False) -> AccountBalance:
    with open(path, newline="") as csvfile:
        ...
        fieldLen = len(_EtradePosition._fields)
        positions = (
            _EtradePosition._make(r[0:fieldLen]) for r in reader if len(r) >= fieldLen
        )
...
class _EtradeTransaction(NamedTuple):
    date: str
    account: str
    action: str
    symbol: str
...
def _parseOptionTransaction(symbol: str, currency: Currency) -> Option:
    ...
    if not match:
        raise ValueError(f"Could not parse Etrade options symbol: {symbol}")
...
def _parseEtradeTransactionDate(datestr: str) -> datetime:
    return datetime.strptime(datestr, "%m/%d/%Y")


def _forceParseEtradeTransaction(t: _EtradeTransaction, flags: TradeFlags) -> Trade:
    quantity = Decimal(t.quantity)

    totalFees = Decimal(0)
    # Etrade's total fees include commision and fees
    if t.commission:
    ...
    return Trade(
        date=_parseEtradeTransactionDate(t.date),
...
def _parseEtradeTransaction(t: _EtradeTransaction) -> Optional[Activity]:
    ...
    return _forceParseEtradeTransaction(t, flags=flags)
    ...
    with open(path, newline="") as csvfile:
        ...
        return list(
            filter(
                None,
                parsetools.lenientParse(
                    (_EtradeTransaction._make(r) for r in sections[0].rows),
                    transform=_parseEtradeTransaction,
                    lenient=lenient,
                ),
            )
        )
...
class EtradeAccount(AccountData):
